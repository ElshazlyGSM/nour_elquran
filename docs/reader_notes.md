...
## 35. تقوية استقرار الإشعارات (عدم توقف الجدولة)

### المشكلة
- بعد التحويل من نمط `alarm`، صارت الإشعارات تصل أحيانًا وتتوقف أحيانًا.
- لو إشعار واحد فشل أثناء الجدولة (صوت/قناة/بروفايل)، كان ممكن يسبب توقف جزء كبير من باقي الجدولة.

### الحل المطبق
- خدمة الأذان:
  - `lib/features/notifications/adhan/prayer_notification_service.dart`
  - كل عملية جدولة داخل الحلقة صارت محمية `try/catch`:
    - فشل إشعار واحد لا يوقف باقي إشعارات الأيام/الصلوات.
  - تمت إضافة قناة احتياطية داخل `_scheduleEntry`:
    - عند فشل تفاصيل الإشعار أو فشل الجدولة بسبب الصوت/القناة، يتم fallback لقناة آمنة لضمان الاستمرار.
- خدمة الصلاة والسلام:
  - `lib/features/notifications/salawat/salawat_notification_service.dart`
  - إضافة fallback channel للتذكير:
    - `_fallbackNotificationDetails(...)`
  - عند فشل الجدولة في الوضع الأساسي:
    - يتم fallback للوضع `inexactAllowWhileIdle` + القناة الاحتياطية بدل انهيار المسار.
  - الفشل المتكرر لإشعار واحد لا يوقف باقي النافذة المتدحرجة.

### لماذا هذا مهم
- نمنع سيناريو "إشعار واحد فشل ⇒ كل الجدولة وقفت".
- نخلي الخدمة resilient حتى لو:
  - بروفايل صوت غير متاح
  - قناة قديمة/غير صالحة
  - قيود جهاز معيّن على نمط الجدولة
